...
AssemblyCode = [] #List stores assembly code
CurrentState = 'S'
symbolTable = []
lastID = "TACO"
lastInt = 0
...
def changeState(prodRule):
    if(prodRule in possibleStates):
        global CurrentState
        CurrentState = prodRule
    ...
    with open("input.txt", "r") as inputFile:
        ...
        for lexeme in multipleLex:
            ...
            if (tempList[1] == "OPERATOR" or tempList[1] == "SEPERATOR" or tempList[1] == "KEYWORD"):
                ...
                LexemeDeque.append('ID'+tempList[0])
            elif (tempList[1] == 'INT'):
                LexemeDeque.append('INT'+str(tempList[0]))
            else:
        ...
        while (len(SyntaxStack) > 0):
            ...
            if 'ID' in incomingToken:
                lastID = incomingToken[2:]
                incomingToken = 'ID'
            if 'INT' in incomingToken:
                lastINT = incomingToken[3:]
                incomingToken = 'INT'
            ...
            if any(item == terminal for item in terminal_list):
                if (terminal == incomingToken):
                    ...
                    if(CurrentState == 'D' and incomingToken == 'ID'):
                        symbolTable.append(lastID)
                    elif(CurrentState == 'A' and incomingToken == 'ID' and not initalIDCheck):
                        idToPutResultIn = lastID
                        initalIDCheck = True
                    elif(CurrentState == 'A' and incomingToken == 'INT'):
                        print(str(counterAssembly)+'    PUSHI    ' + lastINT)
                        counterAssembly += 1
                    elif(CurrentState == 'A' and incomingToken == 'ID' and initalIDCheck):
                        print(str(counterAssembly) + '    PUSHM    ' + str(int(symbolTable.index(lastID)) + memLocation))
                        counterAssembly += 1
                        if(needToDoMath):
                            print(str(counterAssembly)+"    "+typeOfMathToDo)
                            counterAssembly += 1
                            needToDoMath = False
                    elif(CurrentState == 'A' and incomingToken == '+'):
                        typeOfMathToDo = 'ADD'
                        needToDoMath = True
                    elif (CurrentState == 'A' and incomingToken == '-'):
                        typeOfMathToDo = 'SUB'
                        ...
                        typeOfMathToDo = 'DIV'
                        needToDoMath = True
                    elif(CurrentState == 'A' and incomingToken == ';' and initalIDCheck):
                        print(str(counterAssembly)+'    POPM     ' + str(int(symbolTable.index(idToPutResultIn)) + memLocation))
                        counterAssembly += 1
                        initalIDCheck = False
        ...
        for elm in symbolTable:
            print('{0: <12}'.format(elm) + '{0: <16}'.format(str(symbolTable.index(elm) + memLocation)) +'{0: <12}'.format('Integer'))
